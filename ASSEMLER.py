...
class symbolTable:
    ...
    def validate(self,x):
        if len(x)>2:
            if x[1] in self.types and x[0] not in self.symbol and self.addrs==0:
                size=self.get_size(' '.join(x[2:]),x[1])
                self.symbol[x[0]]=[x[1],"D",self.get_value(x[2:],x[1]),size,"00000000"]
                self.addrs+=size
            elif x[1] in self.types and x[0] not in self.symbol and self.addrs!=0:
                _size=self.get_size(' '.join(x[2:]),x[1])
                self.symbol[x[0]]=[x[1],"D",self.get_value(x[2:],x[1]),_size,('0'*(8-len(hex(self.addrs)[2:])))+(''.join(map(self.changeCase,hex(self.addrs)[2:])))]
                self.addrs+=_size
            elif x[1] in self.res_types and x[0] not in self.symbol and self.addrs==0:
                ...
                _size=self.get_size(x[2],x[1])
                self.symbol[x[0]]=[x[1],"D",[],_size,('0'*(8-len(hex(self.addrs)[2:])))+(''.join(map(self.changeCase,hex(self.addrs)[2:])))]
                self.addrs+=_size
            elif x[0] in self.symbol:
                print "__symbol |<"+x[0]+">| declared once and more"
                sys.exit()
        elif len(x)==2:
            if x[0] in self.extern:
                if x[1] not in self.symbol:
                    ...
                    print "__symbol |<"+x[1]+">| declared  once and more"
                    sys.exit()
...
class opcode_table:
    ...
    def convrt_HEX(self,x):
        for i in range(len(x)):
            if x[i]!='' and x[i]!=None:
                x[i]=hex(int(x[i],2))[2:]
        return x
    def display(self):
        x=zip(map(lambda x:''.join(x),self.opr_opcode),self.convrt_HEX(self.ins_code),self.mem_addr,self.values)
        for i in x:
            y=(i[0],''.join(map(self.changeCase,i[1])),str([i[2]]),i[3])
            addr=self.changeHexAdd(str(self.addrs))
            addr='0'*(8-len(addr))+addr
            tot=''.join(filter(lambda s:s!='[\'\']',y))
            print ' '.join([addr,tot])
            self.addrs+=(sum(map(len,y))/2)

    def GEN_code(self,line,st):
        n=len(line)
        if(n>=3):
            if(n>3):
                line=line[1:]
...
def valid_instructns(lines,st,lt,i):
    flag1=0
    flag2=0
    line=lines[i]
    n=len(line)
    if n==1:
        ...
        if line[2][:6]=="dword[" and line[3][:6]=="dword[":
            print "__> memory to memory  transfer  not possible or something missing  invalid"
            sys.exit()
        elif line[2][:6]=="dword[":
            flag1=0
        elif line[3][:6]=="dword[":
            ...
            if line[1] in st.ins:
                ...
                if line[2] in st.reg:
                    if is_alpha(line[3]) and line[3] not in st.symbol and  line[3] not in st.reg and flag1==1:
                        print "__>symbol <"+line[3]+">  usefull but not defined"
                        sys.exit()
                    ...
                    if is_alpha(line[2]) and line[2] not in st.symbol and line[3] not in st.reg and flag2==1:
                        print "__>symbol <"+line[2]+">  usefull but not defined"
                        sys.exit()
...
#<no insrtuction should be there after main: same line>

lines=map(seperate,lines)
print lines
ad=0
for i in range(1,len(lines)):
    valid_instructns(lines,st,lt,i)
    ot.GEN_code(lines[i],st)
